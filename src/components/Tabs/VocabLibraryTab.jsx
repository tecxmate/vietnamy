...
function FlashcardTestView({ deck, onBack }) {
    ...
    const onTouchEnd = (e) => {
        ...
        // Swipe left/right always triggers know/don't-know (auto-reveals if needed)
        if (dx > 60) advance('know');
        else if (dx < -60) advance('unknown');
        // Small movement = tap to flip/unflip
...
export default function VocabLibraryTab() {
    ...
    if (view === 'test' && activeDeck) {
        return <FlashcardTestView deck={activeDeck} onBack={handleBackFromTest} />;
    }
    if (view === 'detail' && activeDeck) {
        return <DeckDetailView deck={activeDeck} onBack={handleBack} onStudy={handleStudyDeck} />;
    }
    return <MyDecksView onSelectDeck={handleSelectDeck} onStudyDeck={handleStudyDeck} />;
}
